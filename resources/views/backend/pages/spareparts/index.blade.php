                ...
                <table id="example1"  class="table table-bordered table-striped">
                  ...
                  <tbody>
                     ...
                     <tr>
                          ...
                          <td>
                            @if ($admin->can('sparepart.edit'))
                              <a class="btn btn-success text-white" href="{{ route('admin.spareparts.edit', $data->id) }}">Edit</a>
                              @endif
                              @if ($admin->can('sparepart.delete'))
                              <button type="button" class="btn btn-danger text-white" href=""
                              onclick="deleteItem({{ $data->id }})">
                                  Delete
                              </button>
                              <form id="delete-form-{{ $data->id }}" action="{{ route('admin.spareparts.destroy', $data->id) }}" method="POST" style="display: none;">
                                  @method('DELETE')
                                  @csrf
                              </form>
                              @endif
